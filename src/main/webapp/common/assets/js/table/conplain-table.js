...
$(document).ready(function(){
	...
	var atable = $("#product_datatable").dataTable(
				...
				"aoColumns" : [
					...
					},
					{
					  ...
					  "mRender": function (data) {
						  var today=new Date(data.compTime.time);
                    ...
                    buttons: {
                        "保存": {
                            ...
                            callback: function () {
        						...
        						var content=addDialog.find("#content").val();  
		...
		$.ajax({
            ...
            success: function (response) {
                ...
                if (response.success===true){
            		...
            		var row = atable.fnGetData(atable.$("tr.active")[0]);
        			$.ajax({
...
$("#detailButton").on("click", function () {
	...
	var row = atable.fnGetData(atable.$("tr.active")[0]);
	var id = row.compId;
	var _title="投诉单位 - "+row.beComp+" - 详细信息";
	$.ajax({
		type : "GET",
		url : "complaint_getList.do?",
		data:{compId:id},
		async : false,
		cache : false,
		dataType : "html",
...
$("#enableButton").on("click", function () {
	if(atable.$("tr.active").size() == 0){
		$.fn.bdmp.message.warning("请选择记录!");
	}else{
		var compId = "";
		$.each(atable.$("tr.active"),function(i,n){  
            if(i==0){
            	compId += atable.fnGetData(n).id;  
            }else {
            	compId += (","+atable.fnGetData(n).id); 
            } 
        });
		...
		bootbox.confirm({
	        ...
	        callback: function(result) {
	        	if (result) {
	        		$.post("product/putOnPullOff.do",{compId:compId, status:2},function(data) {
						if(data.success){
...
$("#deleteButton").on("click", function () {
	if(atable.$("tr.active").size() == 0){
		...
		bootbox.confirm({
	        ...
	        callback: function(result) {
	        	if (result) {
	        		$.post("complaint_delete.do",{compId:ids},function(data) {
						if(data.success){
...
function renderButtonFn(){
	...
	$.each(atable.$("tr.active"),function(i,n){
		if(atable.fnGetData(n).status == 2){
			enableButtonFlag = false;
		}else if(atable.fnGetData(n).status == 1){
			disableButton = false;
		}
    });
	if(atable.$("tr.active").size()==1){
	...
	if(enableButtonFlag&&atable.$("tr.active").size()>0){
		$("#enableButton").removeAttr("disabled");
	}else{
		$("#enableButton").attr("disabled","disabled");
	}
	if(disableButton&&atable.$("tr.active").size()>0){
...
$("#refreshButton").on("click", function () {
	atable.fnPageChange($.fn.bdmp.page.currentPage(atable)); 
});
